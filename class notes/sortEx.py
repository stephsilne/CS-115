...
def insertV1(L, i):
    ...
    '''asserts the everything up until and not including i is sorted'''
    while 0 < i < len(L):
        '''while i is between 0 and the length of the whole list'''
        if L[i] < L[i-1]:
            '''if the element in question is less than the previous element'''
            swap(L,i,i-1)
            '''swap them'''
            i -= 1
            '''decrement i for the while loop to reiterate'''
        else:
            break
            '''otherwise, break'''
    return L
    '''return the final list after all iterations are complete'''
    assert isSorted(L[0:i+1])
    '''assert that everything up to and including i is sorted'''
...
def search(L, i, x):
    ...
    while j < i:
        ...
        if x >= L[j]:
            '''if the element is less than or equal to L[j]'''
            j += 1
            '''increment j by 1 to see te comparison for other elements'''
        else:
            break
            '''otherwise break out of the loop'''
    return j
    '''return the final index needed'''
...
def insertV2(L, i):
    ...
    srch = search(L,i,L[i])
    '''let srch be the search function using the list, the index, and the element in question at i'''
    while 0 < i < len(L):
        '''while i is between the first and last element'''
        if i > srch:
            '''if i is less than the search index value'''
            swap(L,i,i-1)
            '''swap the index with the previous index'''
            i -= 1
            '''decrement i for other iterative comparisons'''
        else:
            break
        '''break out of loop'''
    return L
...
def letterCounts(fname):
    ...
    file = open(fname,'r')
    dictt = {'a':0, 'b':0,'c':0, 'd':0,'e':0, 'f':0, 'g':0,
             'h':0, 'i':0, 'j':0, 'k':0, 'l':0, 'm':0, 'n':0,
             'o':0, 'p':0, 'q':0, 'r': 0, 's': 0, 't':0, 'u':0, 'w':0, 'x':0,
             'y':0, 'z':0, 'non-letter': 0}
    lst = []
    '''initializes list'''
    for line in file:
        ...
        for char in line:
            ...
            cnt = len(filter(lambda x: x == char, line))
            '''count the number of times each character is in the line'''
            dictt[letter(char)] += cnt
            '''update the dictionary'''
            line = filter(lambda x: x != char,line)
            '''delete those filtered characters for updating'''
    file.close()
    '''close file'''
    for ele in dictt.keys():
        '''for each element in the dict keys'''
        lst += [(dictt[ele],ele)]
        '''append it to th empty list'''
    print(lst)
    '''print the lst'''
