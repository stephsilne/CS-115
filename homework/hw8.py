...
def listOfRunLengths(s):
    ...
    def lORL(s, result, curCount, curVal):
        ...
        if s == '':
            ...
            '''result should equal the current count (curCount) as there was only one element in the string given'''
        elif curVal == s[0]:
            '''if the first element of the original string equals the next element of the string, or first element of the new string'''
            return lORL(s[1:],result,curCount + 1,s[0])
            '''recursively return lORL with the rest of the string, incrementing curCount'''
        else:
            return lORL (s[1:],result + [curCount],1,s[0])
            '''otherwise if they are not the same, keep the curCount as 1, input the previous curCount as that is the amount of times the previous element was in the string, and recursively continue'''
...
def findRunBits(s):
    ...
    runlen = listOfRunLengths(s)
    '''equates runlen variable to the recursive function listOfRunLengths of the input string'''
    avglen = (sum(runlen))/(len(runlen))
    '''defines 'avglen' which equals the sum of all runlengths and the amount of runlengths in the list (length)'''
    n = math.ceil(math.log(avglen,2)) + 1
    '''defines n which is the 1 + log of avglen with base 2'''
    return min(n,7)
    '''returns the minimum of this calculated n value and 7'''
